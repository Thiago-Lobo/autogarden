...
{
	ag_output_mode(port, (!state) ? ON : OFF);
}
...
void Relay::timed_step(bool new_state, uint32_t delta_millis)
{
	...
	timed_step_reference = ag_time_millis();
}

bool Relay::update()
{
	if ((timed_step_delta_millis > 0) && (ag_time_millis() >= timed_step_reference + timed_step_delta_millis))
	{
		toggle();
		timed_step_delta_millis = 0;
